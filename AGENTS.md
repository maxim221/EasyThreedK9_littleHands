...
- If USB drops during SD printing, keep the `LH_END_GCODE_V1` predicted final pose in application state. The operator must be able to explicitly confirm normal print completion and model removal; only after that may `Go to start` use the saved predicted final pose for guarded recovery.
...
- The SD-panel post-print return button must call the same guarded `Go to start` / recovery path as the manual-control button. Do not create a separate fire-and-forget terminal-style movement path in the UI.
