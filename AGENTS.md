...
- If hotend preheat fails after Little Hands lifted Z for clearance, attempt to return to saved start before surfacing the error, but mark home uncertain even if Marlin reports `M114 Z0`. A USB/controller reset can make the raised pose become the new logical zero, so the operator must visually confirm the physical start and press `Save start` again.
