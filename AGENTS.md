...
- Long Little Hands hotend preheat before `M24` must use one host-side blocking `M109` session and passively parse Marlin temperature lines. Do not reintroduce the `M104` plus repeated `M105` polling loop for this stage; it has left this K9 stuck near warm-bed temperature and then non-responsive. On preheat abort, send `M108` before `M104 S0`.
