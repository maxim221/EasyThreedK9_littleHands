...
- If hotend preheat fails after Little Hands lifted Z for clearance, return to saved start before surfacing the error. Never leave the operator one click away from saving a false raised `Z0`.
...
- Auto telemetry must stay quiet briefly after each user USB task. Manual start setup involves repeated jog/save commands on a fragile CH340/hub path; do not immediately resume `M105/M27/M114` polling between those commands.
