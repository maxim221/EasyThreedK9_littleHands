...
- If the nozzle is known to be at saved start (`Z0`) before SD start and the SD file owns its early `M109`, do not run service axis moves before `M24`; select the file and start it from the already-saved physical start. If a legacy `M104`-only file needs host preheat, lift to safe Z clearance before the long preheat, then explicitly return to saved start before `M24`.
- Do not rewrite early startup `M109` to `M104` for new K9 G-code. The stable SD workflow is: require the printer to already be at the saved start, send `M23`/`M24`, and let the SD G-code own the hotend heat wait. Only old already-prepared `M104`-only files need host hotend preheat before `M24`; if that fallback is used, it must be one blocking `M109` session with passive temperature parsing, never the old `M104` plus repeated `M105` polling loop. On host-preheat abort, send `M108` before `M104 S0`.
- UI SD-start paths must block unless the app knows the printer is physically at the saved start. Once that is true, use the no-service-move SD start path (`M23`/`M24`) so the file's initial `G92 X0 Y0 Z0` is executed at the real saved start, not after an extra Little Hands axis dance.
