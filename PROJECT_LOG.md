...
## 2026-06-10 Hotbed Graph Readability
...
## 2026-09-09 Mid-Print USB Loss And Recovery Guards

- HPCTOP.GCO last reported `159432/1464804` bytes at 12:33:31 (10.884%). This corresponds to the second G-code layer, Z0.36, but is not a confirmed physical stopping point. Last temperature at 12:33:26 was T223.92/224, B59.01/60.
- At 12:33:41 Linux reported CH340 USB errors `-71` and repeated hub-port disable/re-enumeration (`disabled by hub (EMI?), re-enabling`). The operator subsequently reported an unfinished, stationary print. No app Stop or thermal error was captured at the loss of communication. SD printing should continue independently of the host; the USB failure alone does not prove why physical printing stopped. Controller reset/hang and electrical/link causes remain hypotheses requiring physical investigation.
- Repeated read-only status probes and one targeted CH340 USB reset did not restore Marlin replies. At the operator's request to abandon the print and return to start, the Stop/heater-off sequence was sent, but no acknowledgement or recovery pose was received. No stale-coordinate return was attempted. After a physical power cycle, communication recovered; the operator jogged the axes and saved the physical start at 18:47:46. Nozzle and bed targets then read zero.
- Fixed misleading live state: locally generated SD messages no longer refresh the sample age; missing telemetry is shown as unknown instead of asserted active printing. Stale header temperatures become unknown. Each real SD, raw XYZ, and temperature sample is persisted with its own timestamp; the current source G-code is archived by SHA-256. State writes use atomic replacement and preserve the previous record on failure.
- Added the translated Recovery / После сбоя / 故障恢复 window with a diagnostic JSON export. Records are evidence, not an exact executable crash checkpoint. No automatic G-code tail, M26 seek, G92 reconstruction, or unfinished-print predicted-end return is generated from old telemetry.
- Pause now requires acknowledged M25, planner drain M400, and captured SD file/cursor/position/temperature. Resume validates fresh state in one serial session, checks the archived source and suspect-upload gate, and consumes the pause record durably before sending M24. Reconnect/restart additionally requires the operator to confirm no printer power reset or physical movement. A changed cursor/pose/file, missing reply, cold heater, or lost target blocks resume.
- Stop independently attempts each shutdown command and records whether shutdown was acknowledged. An unconfirmed stop cannot create a trusted stopped pose or live-return option. A live-session-only stopped return is never restored across an app restart.
- Normal polling now reuses one USB session per cycle, closes it before any completion operation, and backs off on silence. Status/metrics inspection no longer mounts the SD card. Firmware reset/error messages invalidate the retained pause and are recorded for diagnosis. These software changes reduce unnecessary USB traffic; they do not establish that the electrical/firmware cause of this incident is solved.
- Added offline runtime and UI coverage, including interrupted atomic writes, wrong/cold resume states, duplicate-start prevention, failed heater-off acknowledgements, one-session polling, real sample ages, language switching, and control visibility. Updated all three in-app guides and recovery rules. Physical print validation is recorded separately after the requested retry.
- Validation passed: required Python compilation, workflow regressions including 12 recovery tests, 12 isolated UI tests across Russian/English/Chinese and both supported window sizes, local documentation links, and `git diff --check`. HPCtop's existing prepared G-code validates at 226C hotend / 60C bed; restarting it requires an operator-confirmed clean bed.

## 2026-09-09 HPCtop Retry: USB Loss At Hotend Heat Start

- The operator confirmed a clean bed and requested HPCtop printing. The current physical start had been saved after the earlier confirmed power cycle; the app's next-start confirmation was accepted on that basis. HPCTOP.GCO remained 1,464,804 bytes with the validated 226C hotend / 60C bed profile.
- The app started host bed preheat at 19:19:41. Marlin B rose smoothly from 25.8C to 59.0C at 19:30:10 with target 60C and positive bed output, satisfying the existing 1C readiness margin. This confirms that reported warmup on this attempt, not surface calibration or completed-print stability.
- The acknowledged clearance lift reported raw X0 Y0 Z10 at 19:30:15. Initial hotend temperature was 42.6C with target/output zero. At 19:30:19 the staged preheat sent M104 S60. At 19:30:20 Linux recorded the CH340 disconnecting and re-enumerating from USB device 44 to 45 on path 1-4.4.4. The serial read failed before heat could be proven; M23/M24 were not sent and no print began.
- The port was absent during the attempted relative Z return and all three heater cleanup attempts. Little Hands retained the dedicated 10mm failed-preheat-lift marker, invalidated home, and required another power cycle. No blind return, new zero, or automatic start was attempted.
- After the port reappeared, a separate heater-shutdown probe received garbled `echo:Unknown command` responses followed by `ok`. These did not establish shutdown. After ASCII resynchronization, M108, M104 S0, and M140 S0 received clean acknowledgements, followed by a complete M105 report: T41.23/0.00, B55.16/0.00, @0, B@0. The operator was asked to leave printer power off and not move axes or save the raised pose.
- Fixed the exposed serial-helper defect: unknown-command and damaged replies now fail even with a trailing `ok`; acknowledgements must be actual `ok` lines. The existing M114 exception requires a complete XYZ report. Offline tests cover the observed corrupted replies and prevent a service move after a rejected coordinate-mode command. All three manuals now state the power-off action when heater shutdown is unconfirmed.
- This reproduction places another physical USB disconnect immediately after the hotend heat command. It does not establish whether power, cabling/hubs, interference, or controller behavior caused it, nor prove that this was the cause of the earlier mid-print stop. Printing remains pending hardware recovery; do not describe this retry as a successful print or the underlying fault as fixed.
- Validation of the acknowledgement fix passed: required Python compilation, workflow regressions with 15 recovery tests, 13 isolated GUI/manual tests, and `git diff --check`. The print-start failure above is physical validation evidence, not a passing print test.

## 2026-09-09 Cancellable Preheat And Raised-Start Guard

- The operator manually found the port, saved start at 19:41:28, confirmed the next-start power cycle, and started another bed preheat at 19:41:40. They then reported that the nozzle was still above the bed and neither lowering it nor closing the app was possible. Save start had silently discarded the previous 10mm failed-lift marker, while the global USB busy guard also disabled Stop throughout the long warmup.
- To stop the already running version, its worker process was terminated before any SD start or second clearance lift. The now-unowned serial port received M108, M104 S0, and M140 S0. A subsequent complete M105 confirmed T34.86/0, B47.31/0, @0, B@0. The known previous 10mm lift was undone with the existing acknowledged relative-return helper, followed by acknowledged G92 X0 Y0 Z0/M114. The operator was asked to visually verify the resulting height; the next-start power-cycle gate remains set. No print began.
- During preheat the existing Stop button now becomes Cancel preheat. Its handler sets a cancellation event for the serial worker, so no competing serial connection is opened. Bed waiting, hotend stages, and final M109 respond to cancellation; M109 receives M108 before heater-off cleanup. Known lift recovery and the next-start gate remain intact.
- Closing during preheat requests cancellation. The app stays open until the worker finishes and heater shutdown is verified; if shutdown remains unknown it stays open with a power-off instruction. Manual movement and port controls remain locked until cleanup completes. Upload and other USB operations retain their existing closure guards.
- Bed preheat now retains one serial connection instead of repeatedly opening and closing it for every temperature poll. Failure cleanup can wait for USB re-enumeration and retry only heater-off commands against the identified printer, using VID/PID plus serial number or USB location. It requires a fresh complete report with zero targets and outputs, including the delayed bed-PWM case seen during manual cleanup. Missing, partial, non-finite, or ambiguous-printer state is not confirmation. No automatic heating restart or axis movement is added after reconnection.
- Save start now asks a default-No physical-position confirmation when a failed-lift marker exists. Declaring zero uses the acknowledged serial helper. This prevents the observed single-click loss of the raised-pose warning; software coordinates still cannot independently measure the nozzle-to-bed gap.
- Added offline checks for cancellation during bed heat, staged hotend heat, and blocking M109; prevented SD start; single-session bed warmup; delayed USB re-enumeration and PWM shutdown; wrong/ambiguous device rejection; translated cancellation controls; close-after-cleanup; and protection of the failed-lift marker. Updated all three manuals and repository rules.
- Read-only USB inspection still shows the printer behind two Genesys hubs. Runtime autosuspend is already disabled for the CH340 and both hubs. No firmware, heater limits, USB power policy, or cabling was changed. The underlying physical disconnect remains unproven and requires a controlled follow-up once the physical start is confirmed.
- Final validation passed: required Python compilation; workflow regressions including 20 recovery/preheat tests; 16 isolated UI tests, including cancellation-button text fit and reachability at 1080x680 and 1280x780 in all three languages; and `git diff --check`. The new cancellation flow was tested without printer serial access. A new physical print has not been started.

## 2026-09-09 HPCtop Start Confirmed After Preheat Fixes

- Delayed operator form replies clarified the prior incidents: at the 19:41 retry Save start was pressed while the nozzle was still raised; no USB cable, hub, or power manipulation occurred during the 19:30 disconnect. The operator subsequently confirmed that the controlled 10mm return restored the correct physical height and that the next 5–10 second power cycle was completed without moving axes.
- Reconnected the unique CH340 at /dev/ttyUSB0 after that power cycle. Fresh reports showed both heater targets and outputs at zero. Saved the confirmed physical start through the acknowledged helper at 20:06:53; raw XYZ and step counts were zero. The operator accepted the app's power-cycle gate at 20:07:12.
- HPCtop preheat started at 20:07:14. The single bed-preheat connection remained open, and Marlin B rose from 28.4C to 59.0C with target 60C at 20:17:38. The cancellation button remained available. The 10mm nozzle clearance was acknowledged at 20:17:43.
- Hotend stages 60/100/150/200C ran without the previous immediate USB disconnect. After a slow early rise, temperature climbed quickly and final M109 confirmed 226.1/226C at 20:21:26. The known relative lift was undone; M114 confirmed raw X0 Y0 Z0 before SD selection. M24 was sent for HPCTOP.GCO at 20:21:40.
