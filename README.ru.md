...
- после штатного завершения печати можно использовать кнопку `Вернуть к старту` в SD-блоке; она запускает тот же guarded recovery, что и `Go to start`, и требует очищенный стол
