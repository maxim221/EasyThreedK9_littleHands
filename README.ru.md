...
- после штатного завершения печати можно использовать кнопку `После печати: к старту` в SD-блоке; она запускает тот же guarded recovery, что и `К сохранённому старту`, и требует очищенный стол
