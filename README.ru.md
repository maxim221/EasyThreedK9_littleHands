...
Текущий публичный baseline — это защищённый второй `K9`.
...
- первый экспериментальный принтер и второй защищённый `K9` уже вели себя по-разному

Если нужно аккуратно принять и не угробить второй такой же принтер, используй:
...
2. Нажимает `Запомнить старт`.
3. Приложение даёт принтеру `G92 X0 Y0 Z0`.
4. `К старту` возвращает принтер в этот логический ноль в рамках чистой текущей сессии.
...
- после неудачного старта печати надёжнее всего обычно:
  ...
  - снова нажать `Запомнить старт`

## Внешний Warm Mat / Hotbed

Проверенный второй принтер использует внешний `warm mat / hotbed`.
