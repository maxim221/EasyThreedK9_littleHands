...
Подключение, печать, нагрев и возврат описаны в [кратком руководстве](USER_GUIDE.ru.md). Тот же текст показывает кнопка руководства приложения. Пути определяются относительно репозитория; папка docs/ должна оставаться рядом с tools/.
...
Перед установкой ярлыка исправьте его поля Exec, Path и Icon под путь к вашей копии репозитория.
...
Известные особенности проверенного K9:

- первая ступень прогрева hotend может идти медленно и сопровождаться тихими щелчками перед резким ростом температуры
- после печати каретка головы влево/вправо может механически залипнуть; освободи или проверь её перед тем, как снова доверять home
