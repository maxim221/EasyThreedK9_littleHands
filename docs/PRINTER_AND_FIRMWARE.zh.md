...
- 在 `M24` 前，Little Hands 总是先用一次 host-side `M109` 证明 hotend 已经加热，并被动读取温度行；如果加热没有确认，就不会发送 `M24`，也不应出现冷态运动
...
- 对于 SD 上已经存在且仍包含早期阻塞 `M109` 的文件，Little Hands 仍会在 `M24` 前预热 hotend；文件中的 `M109` 保留为额外安全等待，但应用不再把它当作唯一的加热门槛
- 在完成、停止、hard stop 或失败的 SD 打印/启动之后，下一次 SD 启动需要明确确认打印机已经断电重启 `5–10` 秒并重新保存起点；单独点击 `Save start` 不会清除这个保护
- 新的 Little Hands 文件不应再把早期 `M109` 改写为 `M104`；`M24` 前的 host-side 预热必须使用一次阻塞 `M109` 会话并被动读取温度行。不要把它改回 `M104` 加重复 `M105` 轮询；这个模式已经让这台 K9 停在接近热床温度并随后不响应。
...
- 旧的已准备 `M104`-only 文件也被支持：应用在 SD 启动前用 host-side `M109` 预热热端
...
9. 从 SD 启动打印。不需要手动预热热端：Little Hands 会先用 host-side `M109` 预热 hotend，把喷嘴返回保存起点，然后才发送 `M23`/`M24`。
10. 如果文件是通过当前 Little Hands 上传或由内置 helper 导出的，早期 `M109` 会保留在 SD 文件中作为额外安全等待；这是正常路径。
...
打印完成后，应用会阻止重复 SD 启动，直到这个恢复流程和断电重启都被确认。
