...
- SD 启动会被阻止，除非应用确认打印机实际停在已保存的 `X0 Y0 Z0`；一旦起点已确认，Little Hands 不会在 `M24` 前做额外的服务轴移动
- 如果旧的 `M104`-only 文件需要 host-side 预热，而 Little Hands 为安全加热抬起了喷嘴，应用会在 `M24` 前明确把喷嘴返回保存起点；如果预热失败，会先用相同距离的相对 Z 向下移动撤销这次抬升，然后再显示错误
...
- 对于 SD 上已经存在且仍包含早期阻塞 `M109` 的文件，Little Hands 不会在 `M24` 前再做一次 host-side 预热：如果打印机已确认在保存起点，应用只会选择文件并启动 SD，让 G-code 自己完成 hotend 加热等待
...
9. 从 SD 启动打印。不需要手动预热热端。如果所选 SD 文件包含自己的早期 `M109`，Little Hands 会从已经确认的保存起点只发送 `M23`/`M24`，加热等待在 G-code 内部完成。
