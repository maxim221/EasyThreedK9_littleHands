...
打印中断时请参阅[恢复说明](USER_GUIDE.zh.md)。USB 断开本身不能证明 SD 打印停止。应用保存带时间的采样及原始文件副本；只有已确认且仍保留的暂停，才能在重新检查文件、位置和温度后继续。重启后若实际位置未知，必须手动设置起点。2026-09-09 的 HPCTOP 故障及尚未确定的根本原因记录在 [PROJECT_LOG](../PROJECT_LOG.md)。
...
- 如果喷头左右轴只是在打印后卡住，几次短 jog 后又恢复正常，应先按机械滑车问题处理；不要用提高 recovery speed 或更强的返回移动来掩盖它
...
9. 从 SD 启动打印。不需要手动预热热端：Little Hands 会先用分段 `M104` 和最终 host-side `M109` 预热 hotend，把喷嘴返回保存起点，然后才发送 `M23`/`M24`。
