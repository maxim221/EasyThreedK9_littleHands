...
- Cura active-machine preference: in `~/.config/cura/5.11/cura.cfg`, `[cura] active_machine` must be `lilHands_k9_warmmat`. If it is the old `lilHands`, Cura can save a file that looks valid but has no `;LH_EXPERIMENTAL_HOTBED_TARGET:60` marker and therefore will not heat the controlled bed.
...
- Controlled hotbed start target: `60C`, emitted only as `;LH_EXPERIMENTAL_HOTBED_TARGET:60` plus non-blocking `M140 S60`
- This local default changed at the operator's request on 2026-09-09. The app/helper target cap is `60C`, and bed preheat waits up to `15 minutes`. Re-slice and upload existing G-code to change its target. Edit live Cura settings with Cura closed; restart Little Hands when the printer is idle.
...
- K9 service-motion limit: the verified `LH-v5` baseline can still have EEPROM/Marlin service dynamics such as `M201 X1000 Y1000` and `M204 T1000`, which are too aggressive for manual and recovery moves on the tiny mechanics. Keep print travel acceleration at or below `200 mm/s^2`; Little Hands leaves recovery motion in the soft `M204 T80` service-idle state, moves long bed service moves around `F240`, uses the validated `F600` / `M204 P80 T80` manual bed jog context, keeps head left/right service/recovery around `F900`, and uses the smoother one-move `F600` / `M204 P80 T80` diagnostic context for manual head left/right jog. Manual X jog declares a local neutral `G92 X50` before the selected step so stale negative Marlin X does not accumulate after a skipped move.
- Manual X jog acknowledgement is not treated as proof of physical X motion; if the head sticks, verify the carriage by eye and re-save the start before printing.
...
- Cura material bed target remains `0C`; controlled-hotbed files must be explicitly marked `;LH_EXPERIMENTAL_HOTBED_TARGET:60` and use only non-blocking `M140 S60`
