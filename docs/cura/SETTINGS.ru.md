...
- ранний блокирующий `M109` должен оставаться в SD-файле; Little Hands запускает SD-файл и отдаёт ожидание нагрева самому G-code, не дублируя host-preheat перед `M24`
- старые уже подготовленные `M104`-only файлы лучше пересоздать; приложение оставляет host-preheat fallback только для совместимости с ними
