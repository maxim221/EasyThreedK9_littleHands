...
- ранний блокирующий `M109` должен оставаться в SD-файле; Little Hands всё равно сначала подтверждает нагрев hotend через host-side `M109` перед `M24`, а файловый `M109` остаётся дополнительной страховкой
- старые уже подготовленные `M104`-only файлы тоже поддерживаются тем же host-preheat перед `M24`
