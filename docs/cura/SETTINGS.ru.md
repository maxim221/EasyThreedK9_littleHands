...
Это текущий проверенный baseline для EasyThreeD K9 / Little Hands. Настройки специально консервативные: медленная PLA-печать, manual-zero start, а управляемый hotbed включается только явной меткой Little Hands.
...
- Стандартная Cura bed temperature: `0C`; управляемый hotbed запрашивается только меткой в start G-code ниже
...
- Ограничение сервисных движений K9: verified `LH-v5` baseline всё ещё может иметь в EEPROM/Marlin service dynamics вроде `M201 X1000 Y1000` и `M204 T1000`, что слишком резко для ручных и recovery-движений маленькой механики. Для печати держи travel acceleration не выше `200 mm/s^2`; для длинных service/recovery-движений Little Hands оставляет мягкий service-idle режим `M204 T80` и ведёт стол примерно `F240`, для ручного jog стола использует проверенный контекст `F600` / `M204 P80 T80`, service/recovery головы влево/вправо держит около `F900`, а ручной jog головы влево/вправо использует более мягкий диагностический one-move контекст `F600` / `M204 P80 T80`. Перед ручным X-jog приложение объявляет локальный нейтральный `G92 X50`, чтобы stale отрицательный X Marlin не накапливался после пропущенного движения.
- Подтверждение ручного X-jog не считается доказательством физического движения X; если голова залипает, проверь каретку глазами и заново сохрани старт перед печатью.
- Не компенсируй механическое залипание головы влево/вправо после печати повышением Cura travel speed или recovery speed. Сначала освободи / проверь ось; если после коротких jog она ходит нормально, это полевая механика, а не цель для ускорения слайсера.
